...
def step(model):
    sampler = model.dataset.sampler(batch_size=model.cf_batch_size, pos_mode=model.sampler_mode)
    bars = tqdm(sampler, desc='CF', dynamic_ncols=True, leave=False, total=model.num_cf_batches)
    loss_total = 0
...
def train(model):
    for epoch in trange(1, args.epochs + 1, desc='epochs', dynamic_ncols=True):
        ...
        with torch.no_grad():
            model.train_g.edata['w'] = model.compute_attention(model.train_g)
        loss = step(model)
...
if __name__ == '__main__':
    ...
    # initialization of this class processes the data
    DataLoader(args.data_path, args.data_name, logger)
